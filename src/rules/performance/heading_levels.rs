...
impl Rule for HeadingLevels {
    ...
    fn run(&self, program: &Program, _semantic: &Semantic, source_text: &str) -> Vec<RuleFinding> {
        let mut collector = HeadingCollector { findings: Vec::new(), source: source_text, prev_level: 0 };
...
impl<'a> Visit<'a> for HeadingCollector<'a> {
    fn visit_jsx_opening_element(&mut self, el: &oxc_ast::ast::JSXOpeningElement<'a>) {
        ...
        if self.prev_level > 0 && level > self.prev_level + 1 {
            self.add_finding(el.span.start as usize,
                format!("Heading level skipped: h{} followed by h{}", self.prev_level, level));
        }
