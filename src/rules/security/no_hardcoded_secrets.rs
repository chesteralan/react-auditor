...
const SUSPICIOUS_NAMES: &[&str] = &[
    ...
    "access_token", "private_key", "privateKey",
];
...
impl Rule for NoHardcodedSecrets {
    ...
    fn run(&self, program: &Program, _semantic: &Semantic, source_text: &str) -> Vec<RuleFinding> {
        let mut collector = SecretCollector { findings: Vec::new(), source: source_text };
        ...
        if is_suspicious_name
            && let Some(init) = &decl.init
                && let oxc_ast::ast::Expression::StringLiteral(s) = init {
                    ...
                    if val.len() > 8 && val.chars().any(|c| c.is_ascii_punctuation() || c.is_ascii_digit()) {
